...
<div class="container-fluid px-4 py-4 mx-auto">
    <div class="mb-4 flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $promo->judul_promo }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
        <form action="{{ route('admin.promo.update', $promo->id_promo) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('tgl_start') border-red-500 @enderror" id="tgl_start" name="tgl_start" value="{{ old('tgl_start', $promo->tgl_start->format('Y-m-d')) }}" required placeholder="Pilih tanggal mulai">
                            @error('tgl_start')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('tgl_end') border-red-500 @enderror" id="tgl_end" name="tgl_end" value="{{ old('tgl_end', $promo->tgl_end->format('Y-m-d')) }}" required placeholder="Pilih tanggal berakhir">
                            @error('tgl_end')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex">
                                ...
                                <input type="number" class="flex-1 px-3 py-2 border focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('nilai_diskon') border-red-500 @enderror rounded-r-md" id="nilai_diskon" name="nilai_diskon" value="{{ old('nilai_diskon', $promo->nilai_diskon) }}" required min="1" placeholder="Nilai diskon">
                    ...
                    <div>
                        ...
                        <select class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('status') border-red-500 @enderror" id="status" name="status" required {{ $promo->status === 'berakhir' ? 'disabled' : '' }}>
                            <option value="aktif" {{ old('status', $promo->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="nonaktif" {{ old('status', $promo->status) == 'nonaktif' ? 'selected' : '' }}>Non-aktif</option>
                            @if($promo->status === 'berakhir')
                                <option value="berakhir" selected>Berakhir</option>
                            @endif
                        </select>
                        @if($promo->status === 'berakhir')
                            <input type="hidden" name="status" value="berakhir">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Status tidak dapat diubah karena promosi telah berakhir.</p>
                        @endif
                        @error('status')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select class="select2 w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('produk_ids') border-red-500 @enderror" id="produk_ids" name="produk_ids[]" multiple required>
                            @foreach($produks as $produk)
                                <option value="{{ $produk->id_produk }}" {{ in_array($produk->id_produk, old('produk_ids', $selectedProduks)) ? 'selected' : '' }}>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mt-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex">
                                ...
                                <span class="text-gray-800 dark:text-gray-200 font-medium">{{ $promo->user->nama_user }}</span>
                            </div>
                            ...
                            <div class="flex">
                                ...
                                <span class="text-gray-800 dark:text-gray-200 font-medium">{{ $promo->produks->count() }} produk</span>
                            </div>
                        </div>
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-700 hover:bg-indigo-800 text-white rounded-md text-sm transition duration-300" {{ $promo->status === 'berakhir' ? 'disabled' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var minDate = "{{ $promo->status === 'berakhir' ? '' : 'today' }}";
