...
<div class="container-fluid px-4 py-4 mx-auto">
    <div class="mb-4">
        <h1 class="text-2xl font-semibold">Galeri Foto ({{ count($galeris) }})</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-3 py-4 whitespace-nowrap">
                            <img src="{{ asset('storage/' . $galeri->gambar_galeri) }}" class="gallery-image shadow" alt="{{ $galeri->nama_galeri }}" onclick="window.open('{{ asset('storage/' . $galeri->gambar_galeri) }}', '_blank')">
                        </td>
                        <td class="px-3 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $galeri->nama_galeri }}</div>
                        </td>
                        <td class="px-3 py-4 whitespace-nowrap">
                            @php
                                $statusClass = '';
                                $statusText = $galeri->status->nama_status;
                                
                                if(strtolower($galeri->status->nama_status) == 'aktif') {
                            ...
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.away="open = false" x-cloak class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 z-40">
                                    <div class="py-1" role="menu" aria-orientation="vertical">
                                        <a href="{{ route('admin.galeri.edit', $galeri->id_galeri) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" role="menuitem">
                                            <i class="bi bi-pencil mr-2"></i> Edit
                                        </a>
                                        <form action="{{ route('admin.galeri.destroy', $galeri->id_galeri) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto ini?');">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class=" sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700 dark:text-gray-400">
                        Showing <span class="font-medium">{{ count($galeris) }}</span> photos
                    </p>
                </div>
            </div>
        </div>
