...
<div class="container max-w-2xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
        ...
        <form action="{{ route('admin.download.update', $downloadItem->id_download_item) }}" method="POST" enctype="multipart/form-data" id="uploadForm">
            @csrf
            @method('PUT')
            <div class="p-6">
                <div class="mb-6">
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" 
                           id="nama_item" name="nama_item" value="{{ old('nama_item', $downloadItem->nama_item) }}" required>
                    @error('nama_item')
                ...
                <div class="mb-6">
                    ...
                    <div class="current-file flex items-center">
                        @php
                            $extension = pathinfo($downloadItem->path_file, PATHINFO_EXTENSION);
                        ...
                        @endphp
                        <i class="bi bi-{{ $icon }} {{ $iconClass }} text-2xl mr-3"></i>
                        <div>
                            <a href="{{ route('admin.download.file', $downloadItem->id_download_item) }}" target="_blank" class="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">
                                {{ basename($downloadItem->path_file) }}
                            </a>
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Uploaded: {{ $downloadItem->tgl_upload->format('d M Y') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" 
                        ...
                        <option value="Active" {{ old('status', $downloadItem->status) == 'Active' ? 'selected' : '' }}>Active</option>
                        <option value="Inactive" {{ old('status', $downloadItem->status) == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
