...
<div class="container-fluid px-4 py-4 mx-auto">
    <div class="flex justify-between items-center mb-4">
        <div>
            <h1 class="text-2xl font-semibold">Edit FAQ</h1>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <form action="{{ route('admin.faq.update', $faq->id_faq) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="judul" id="judul" class="border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm w-full p-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Masukkan pertanyaan" value="{{ old('judul', $faq->judul) }}" required>
                @error('judul')
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="6" class="border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm w-full p-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Masukkan jawaban dari pertanyaan tersebut" required>{{ old('deskripsi', $faq->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
