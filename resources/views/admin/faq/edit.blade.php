...
<script>
    ...
    function fetchFAQ() {
        ...
        axios.get(`/api/admin/faqs/${id}`)
            .then(response => {
                if (response.data.status === 'success') {
                    const faq = response.data.data;
                    
                    // Fill form with FAQ data
                    document.getElementById('judul').value = faq.judul;
                    document.getElementById('deskripsi').value = faq.deskripsi;
            ...
            .catch(error => {
                ...
                showAlert('error', 'Error loading FAQ: ' + (error.response?.data?.message || error.message));
    ...
    function submitForm(event) {
        ...
        // Submit to API
        axios.post(`/api/admin/faqs/${id}`, formData)
            .then(response => {
                if (response.data.status === 'success') {
                    showAlert('success', 'FAQ berhasil diupdate');
                    // Redirect after a short delay
                    setTimeout(() => {
                        window.location.href = '{{ route("admin.faq.index") }}';
                    }, 1500);
                } else {
                    showAlert('error', 'Gagal mengupdate FAQ');
                }
            })
            .catch(error => {
